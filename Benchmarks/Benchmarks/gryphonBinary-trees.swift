private class GryphonTreeNode {
    var left, right: GryphonTreeNode?
    init(left: GryphonTreeNode?, right: GryphonTreeNode?) {
        self.left = left
        self.right = right
    }
    func check() -> Int {
...
private func createTree(_ depth: Int) -> GryphonTreeNode? {
    if depth > 0 {
        let node = GryphonTreeNode(left: createTree(depth-1),
                                   right: createTree(depth-1))
        return node
    } else {
        let node = GryphonTreeNode(left: nil, right: nil)
        return node
    }
}
class GryphonBinaryTrees {
    static func run(n: Int = 10) -> String {
        ...
        var results = [String](repeating: "", count: (maxDepth-minDepth)/2+1) // kotlin: ignore
        // insert: var results = Array<String>(size = (maxDepth-minDepth)/2+1) { "" }
